...
| 3 | Timing determinism | Spec only | Formal property `no_data_dependent_latency` (verification-plan.md §2.1) named but not written; no RTL to check it against yet. — **2026-09-22 (issue #24):** the property is now **written and qualified** — `verification/formal/no_data_dependent_latency.sv`, a port-only SVA monitor over DR 0001's ratified latency table, with a cold-start Yosys+SMTBMC harness (`verification/formal/run-no-data-dependent-latency.sh`), positive/negative control fixtures, and an append-only evidence record (`verification/records/no-data-dependent-latency/`): BMC pass on the conformant timing-contract fixture at depth 30 with all non-vacuity covers reached, k-induction attempted (does not close against a port-only monitor — see the property header), and a deliberately data-dependent `WAIT` early-out mutant caught within 5 cycles. §2.2's `pin_write_latency` is explicitly deferred with its reason (verification-plan §2.2 note). **Status unchanged in substance: still unmet** — the DUT in those runs is a qualification fixture standing in for the core (issue #18 is still in flight); row 3 closes only when this same property, byte-identical, passes against #18's real core RTL. |
